...
export type FinalAttemptAnswers =
  Record<
    string,
...
function buildFinalAnswers(
  attempt: Attempt,
  questions: Question[],
  submittedAnswers: FinalAttemptAnswers
):
  | {
      success: true;
   ...
   * keep answers that may already exist in Firestore.
   * The new quiz-session will send the complete local answer set,
   * which overwrites these values before final submission.
   */
  const nextAnswers:
  ...
  for (
    ...
    of Object.entries(submittedAnswers)
  ) {
    if (
      !attempt.questionOrder.includes(questionId)
    ) {
      return {
        success: false,
        messageKey:
          "attempts.submit.invalidQuestion",
        message:
          "One or more submitted answers belong to an invalid question.",
      };
    }
    ...
    if (question.type === "qcm") {
      ...
      if (
        ...
        return {
          success: false,
          messageKey:
            "attempts.submit.invalidChoice",
          message:
            "One or more QCM answers are invalid.",
        };
      }
    ...
    if (
      ...
      if (
        !Array.isArray(
          rawAnswer.selectedChoiceIndexes
        )
      ) {
        return {
          success: false,
          messageKey:
            "attempts.submit.invalidMultipleChoice",
          message:
            "One or more multiple-choice answers are invalid.",
        };
      }
    ...
    if (
      typeof rawAnswer.developmentAnswer !==
      "string"
    ) {
      return {
        success: false,
        messageKey:
          "attempts.submit.invalidDevelopment",
        message:
          "One or more development answers are invalid.",
      };
    }

    nextAnswers[questionId] = {
      developmentAnswer:
        rawAnswer.developmentAnswer.slice(
          0,
          MAX_DEVELOPMENT_ANSWER_LENGTH
        ),
      updatedAt: now,
...
> {
  ...
  if (!finalAnswersResult.success) {
    return {
      ...
      messageKey:
        finalAnswersResult.messageKey,
      message:
        finalAnswersResult.message,
    };
  }

  const finalAnswers =
    finalAnswersResult.answers;
